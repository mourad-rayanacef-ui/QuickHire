...
export default function JobDetailsPage() {
 ...
 const applyMutation = useMutation({
  mutationFn: async ({ userId, jobId }) => {  // ✅ Accept params
    const accountType = localStorage.getItem("accountType");
    ...
    console.log('📤 JobDetails sending:', { userId: Number(userId), jobId: Number(jobId) });
 ...
 const handleApply = () => {
  ...
  if (!token) {
    showNotification("You must be logged in to apply for a job.", 'error');
    navigate('/SignIn');
    return;
  }
  ...
  if (accountType !== "user") {
    showNotification("Only users can apply for jobs.", 'error');
    return;
  }
  ...
  if (!currentUserId) {
    showNotification("Unable to retrieve your user information. Please log in again.", 'error');
    navigate('/SignIn');
    return;
  }

  // ✅ DEBUG LOG
  console.log('📦 JobDetails apply payload:', { 
  ...
  // ✅ Call the mutation with the fresh userId
